...
export default function CreameryPage() {
  ...
  useEffect(() => {
    const fetchCreamery = async () => {
      setIsLoading(true)
      try {
        const data = await getProducts({ category: "creamery" })
        setProducts(data.products.filter(p => p.isActive))
      } catch (error) {
        console.error("Failed to fetch creamery products:", error)
        toast({
          title: "Error",
          description: "Could not load creamery products.",
          variant: "destructive",
        })
      } finally {
        setIsLoading(false)
  ...
  return (
    <div className="container mx-auto px-4 py-16">
      <div className="mb-8">
        <h1 className="text-4xl font-bold mb-4">Creamery</h1>
        <p className="text-gray-600 text-lg">Fresh cream, butter, and dairy products</p>
      </div>
      ...
      {sorted.length === 0 ? (
        <div className="text-center py-8">
          <p className="text-gray-500">No creamery products available.</p>
        </div>
      ) : (
              ...
              <Card key={product.id} className="overflow-hidden transition-all hover:shadow-lg">
                ...
                <CardHeader className="p-4 pb-0">
                  <div className="flex justify-between items-start">
                    <span className="text-xs text-amber-600 font-medium uppercase tracking-wide">Creamery</span>
